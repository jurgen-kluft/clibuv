...
{
    server_context_t *ctx = (server_context_t *)client->data;
    if (nread > 0)
    {
        ...
        if (pkt)
        {
            ...
            connection_info_t *info = connection_manager_find_or_create(ctx->conn_mgr, ip, port, 0, CONN_TCP);
    ...
    else if (nread == UV_EOF)
    {
        ...
        uv_ip4_name((struct sockaddr_in *)&addr, ip, sizeof(ip));
        int port = ntohs(((struct sockaddr_in *)&addr)->sin_port);
        connection_manager_mark_disconnected(ctx->conn_mgr, ip, port);
...
void on_new_tcp_connection(uv_stream_t *server, int status)
{
    ...
    if (uv_accept(server, (uv_stream_t *)client) == 0)
    {
        ...
        connection_info_t *info = connection_manager_find_or_create(ctx->conn_mgr, ip, port, local_port, CONN_TCP);
        info->handle            = client;
...
void on_udp_recv(uv_udp_t *handle, ssize_t nread, const uv_buf_t *buf, const struct sockaddr *addr, unsigned flags)
{
    if (nread > 0 && addr)
    {
        ...
        if (pkt)
        {
            ...
            connection_info_t *info = connection_manager_find_or_create(ctx->conn_mgr, ip, port, 0, CONN_UDP);
            info->last_active       = uv_hrtime();
